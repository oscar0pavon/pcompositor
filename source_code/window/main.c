...
typedef struct xdg_surface CrossDesktopGroup;
...
static void buffer_release(void *data, Buffer *buffer)
{
...
static Buffer * draw_frame(State *state)
{
...
static void surface_configure(void *data, CrossDesktopGroup *surface, uint32_t serial)
{
    State *state = data;
    xdg_surface_ack_configure(surface, serial);

    Buffer *buffer = draw_frame(state);
...
static void registry_global(void *data, Registry *registry,
...
static void registry_global_remove(void *data, Registry *wl_registry, uint32_t name)
{
